...
#define DISPLAY_VER(xe) (DISPLAY_RUNTIME_INFO(xe)->ip.ver)
#define IP_VER(ver, rel)		((ver) << 8 | (rel))
#define DISPLAY_VER_FULL(xe)	IP_VER(DISPLAY_RUNTIME_INFO(xe)->ip.ver, \
				       DISPLAY_RUNTIME_INFO(xe)->ip.rel)
...
#define IS_DISPLAY_STEP(xe, first, last) ({u8 __step = (xe)->display.info.__runtime_info.step; first <= __step && __step <= last;})
...
#define RUNTIME_INFO(xe)		(&(xe)->__runtime
